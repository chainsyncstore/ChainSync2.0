...
async function runAnalyticsInsightScan(): Promise<void> {
  try {
    ...
    for (const store of storesList) {
      ...
      try {
        const insights = await analyticsService.generateInsights(store.id);
        const highPriority = insights.filter((insight) => insight.priority === 'high' || insight.priority === 'critical');
        for (const insight of highPriority) {
          await emitAiInsightAlert({
            ...
            priority: insight.priority === 'critical' ? 'high' : 'medium',
            data: {
        ...
        logger.warn('Failed to emit AI insight', {
          storeId: store.id,
